...
class Terminal {
  constructor() {
    ...
    this.commandHistory = [];
    this.historyIndex = 0;
    this.autoScroll = false; // Disable auto-scrolling to prevent any browser scroll behavior
  ...
  init() {
    ...
    const currentTime = `${pad(hours)}:${pad(minutes)}:${pad(seconds)} ${ampm} on ${month}/${day}/${year}`;

    // Check if the cookie exists
    const cookieExists = document.cookie
      .split(";")
      .some((cookie) => cookie.trim().startsWith("Last login at:"));
    if (!cookieExists) {
      document.getElementById(
        "timeoutput"
      ).textContent = `Last login at: ${currentTime}`;
    } else {
      // Optionally, display the previous login time from the cookie
      const lastLogin = document.cookie
        .split(";")
        .find((cookie) => cookie.trim().startsWith("Last login at:"))
        ?.split("Last login at:")[1]
        ?.trim();
      if (lastLogin) {
        document.getElementById(
          "timeoutput"
        ).textContent = `Last login at: ${lastLogin}`;
      }
    }
    document.cookie = "Last login at:" + currentTime;
    this.setupEventListeners();
    this.focusInput();
    this.ensureTerminalScrollable();
  }

  // Ensure the terminal has proper scrolling setup (internal only)
  ensureTerminalScrollable() {
    if (this.terminalBody) {
      // Ensure the terminal body has proper overflow and height settings
      this.terminalBody.style.overflowY = "auto";
      this.terminalBody.style.overflowX = "hidden";

      // If the terminal doesn't have a fixed height, set one
      const computedStyle = window.getComputedStyle(this.terminalBody);
      if (
        computedStyle.height === "auto" ||
        computedStyle.maxHeight === "none"
      ) {
        // Set a reasonable max height (you can adjust this value)
        this.terminalBody.style.maxHeight = "400px";
      }

      // Prevent any browser scrolling
      this.terminalBody.style.position = "relative";
    }
  }
  ...
  setupEventListeners() {
    ...
    // Focus input when clicking on terminal - but prevent page scrolling
  ...
  focusInput() {
    // Prevent the browser from scrolling when focusing the input
  ...
  scrollToBottom() {
    // Only scroll the terminal container, never the browser
    if (
      this.terminalBody &&
      ...
      requestAnimationFrame(() => {
        // Double-check we're only affecting the terminal element
        if (
          this.terminalBody &&
          typeof this.terminalBody.scrollTop === "number"
        ) {
